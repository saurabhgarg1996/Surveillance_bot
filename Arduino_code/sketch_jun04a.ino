...
void loop() {
   while(flag==1)
 {
    ...
    if(Serial1.read()=='s')
    {
        ...
        servo_gun_x.write(angle_x+15,30,false);
        servo_gun_y.write(angle_y-5,30,false);
        delay(1000);
  ...
  else if (start=='m')
  {
    while(1)
    {
      ...
      servo_x(angle_x,20,false);
      servo_y(angle_y,20,false);
      servo_gun_x(angle_gun_x,20,false);
      servo_gun_y(angle_gun_y,20,false);
      delay(100);
        else if(control =='s')
        {
          gun_trigger();
